...
export class MateriaService {
  ...
  async findMateriasDisponibles(estudianteId: number): Promise<Materia[]> {
    ...
    const departamentosPermitidos = [
      ...
      ...(departamentoBasicas ? [departamentoBasicas.id] : []),
    ].filter((id, index, self) => typeof id === 'number' && self.indexOf(id) === index);

    if (departamentosPermitidos.length === 0) {
      return [];
    }
    ...
    return this.materiaRepository.find({
      ...
      relations: [
        ...
        'comisiones.profesor',
        'comisiones.horarios',
        'comisiones.horarios.materia',
        'comisiones.horarios.comision',
        'correlativasCursada',
        'correlativasCursada.correlativa',
        'correlativasFinal',
        'correlativasFinal.correlativa',
      ],
      order: { nombre: 'ASC' }
    });
  }
